...
#         self.fsa.calculateOffsets(sizeCounter=lambda state: self.getStateSize(state))
    ...
    def _serializeTags(self, tagsMap):
        ...
        for tag, tagnum in sorted(tagsMap.items(), key=_a(lambda tag, tagnum: tagnum)):
    ...
    def serializeQualifiersMap(self):
        label2labelId = dict([
            (u'|'.join(qualifiers), n)
            for qualifiers, n in sorted(
                self.qualifiersMap.items(), key=_a(lambda qs, n: n)
            )
        ])
        ...
        #~ label2labelId = dict([ (u'|'.join(qualifiers), n) for qualifiers, n in sorted(self.qualifiersMap.items(), key=lambda (qs, n): n) ])
        #~ res.extend(self._serializeTags(label2labelId))
        #~ for qualifiers, n in sorted(self.qualifiersMap.items(), key=lambda (qs, n): n):
...
#         segmentationDataSize = len(segmentationRulesData) if segmentationRulesData else 0
    ...
    def getSortedTransitions(self, state):
        return sorted(
            state.transitionsMap.items(),
            key=_a(lambda label, nextState: (-state.label2Freq.get(label, 0), -self.fsa.label2Freq.get(label, 0)))
        )
...
#         self.ARRAY_FLAG = 0b01000000
    ...
    def serializeFSAPrologue(self):
        ...
        sortedLabels = [label for (label, freq) in sorted(self.fsa.label2Freq.items(), key=_a(lambda label, freq: (-freq, label)))]
...
#                 assert offset < 256 * 256 * 256  # TODO - przerobic na jakis porzadny wyjatek
    ...
    def _trimState(self, state):
        ...
        newState.transitionsMap = dict([(label, nextState) for (label, nextState) in state.transitionsMap.items()])
#         newState.transitionsMap = dict([(label, nextState) for (label, nextState) in state.transitionsMap.items() if not label in self.label2ShortLabel or not self.label2ShortLabel[label] in range(1,64)])
    ...
    def _transitions2ArrayBytes(self, state):
        res = bytearray()
        array = [0] * 64
        for label, nextState in state.transitionsMap.items():
