...
#         self.transitionsDataMap[byte] = data
    ...
    def getRegisterKey(self):
        return ( frozenset(self.transitionsMap.items()), tuple(self.encodedData) if self.encodedData else None )
    ...
    def dfs(self, alreadyVisited, sortKey=lambda state: -state[1].freq):
        if not self in alreadyVisited:
            ...
            for _, state in sorted(self.transitionsMap.items(), key=sortKey):
    ...
    def debug(self):
        ...
        for label, s in self.transitionsMap.items():
            print(label, '-->', s.idx)
