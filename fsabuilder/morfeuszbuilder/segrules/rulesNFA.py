...
#         self.transitionsDataMap = {}
    ...
    def dfs(self, visitedStates=set()):
        if not self in visitedStates:
            ...
            yield self
            for _, nextStates in self.transitionsMap.items():
    ...
    def debug(self):
        ...
        for label, nextStates in self.transitionsMap.items():
...
class RulesNFA(object):
    ...
    def _groupOutputByLabels(self, nfaStates):
        ...
        for nfaState in nfaStates:
            for label, nextStates in nfaState.transitionsMap.items():
...
#             dfaState.encodedData = bytearray([1 if weak else 0])
        for (segnum, shiftOrth), nextNFAStates in self._groupOutputByLabels(nfaStates).items():
